      ...
      <div className="bg-white border border-border-color rounded-b-[16px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.05)] mb-8 border-t-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-text-muted text-sm m-0">Atualizado agora (Mock Data)</p>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            <button className="flex-1 md:flex-none border-none cursor-pointer bg-primary-light text-primary px-5 py-3 rounded-[8px] font-bold text-[14px] hover:bg-opacity-80 transition-opacity">
              Atualizar dados
            </button>
            <a href="https://www.instagram.com/eacporciunculadesantana/" target="_blank" rel="noreferrer" className="flex-1 md:flex-none text-center bg-primary text-white px-5 py-3 rounded-[8px] font-bold text-[14px] no-underline hover:bg-primary-dark transition-colors">
              Instagram do EAC
            </a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <MetricCard label="Estoque Físico Total" value={totalFisico} />
          <MetricCard label="Reserva Brinde Total" value={totalReserva} />
          <MetricCard label="Disponível Total" value={totalDisponivel} />
          <MetricCard label="Pedidos Reservados" value={totalReservados} />
          
          <MetricCard label="Disponível Preta" value={totalPretaDisponivel} />
          <MetricCard label="Disponível Azul" value={totalAzulDisponivel} />
          <MetricCard label="Pedidos c/ Alternativa" value={totalAlternativa} />
          <MetricCard label="Pedidos em Reposição" value={totalReposicao} />
        </div>
        ...
        <div className="bg-white border border-border-color rounded-[16px] overflow-hidden">
          <div className="p-4 md:p-[20px] pb-3 border-b border-border-color">
            ...
            <p className="mt-1.5 mb-0 text-text-muted text-[13px]">Resumo consolidado do estoque e das solicitações registradas.</p>
          </div>
            ...
            <table className="w-full min-w-[980px] border-collapse text-[14px]">
              <thead>
                <tr>
                  ...
                  <th className="bg-[#F8F9FA] text-text-muted p-4 text-left font-semibold sticky top-0 border-b border-border-color">Disponível</th>
                  <th className="bg-[#F8F9FA] text-text-muted p-4 text-left font-semibold sticky top-0 border-b border-border-color">Solicitações</th>
                  ...
                  <th className="bg-[#F8F9FA] text-text-muted p-4 text-left font-semibold sticky top-0 border-b border-border-color">Reposições</th>
                </tr>
              </thead>
              <tbody>
                {MOCK_STOCK.map((row, idx) => (
                  <tr key={idx} className="hover:bg-[#FAFAFA] transition-colors border-b border-border-color last:border-0">
